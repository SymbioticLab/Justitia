...
#  exp_mult_wr.sh
#  
# This script is used to test diff num_wr on same data size
#
...
# $4 is the data size
# sudo chmod +x exp_mult_wr.sh; (for permission issue)
...
do
    echo -n "running RDMA WRITE test on $(($multiple * 10)) work requests:  "
    echo $4 >> temp
    counter=0
    while [ $counter -lt $num_run ]
    do
        #echo $(($counter+1))
        ./rdma-client write $1 $2 $4 $(($multiple * 10)) | grep '^\s' | sed "s/$/ $(($multiple * 10))"/ >> temp
        let counter+=1
