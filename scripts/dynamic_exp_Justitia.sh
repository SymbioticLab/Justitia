#!/bin/bash
## This runs on IB
## Adjust MaxStartups in /etc/ssh/sshd_config (e.g., MaxStartups 60) to allow more concurrent ssh connections
...
if [ $1 == "sender" ]; then
  for ((i=1; i<=$num_flows; i++)); do
    ...
    if [[ $cnt -eq $num_flows ]]; then
	  ...
	  for ((j=1; j<=8; j++)); do
        ...
        cmd="export LD_LIBRARY_PATH=/usr/lib64; $ib_write_lat -F -s $lat_size -n $lat_iter -l 1 -t 1 --log_off -p $port $ip_receiver |tee $output"
      ...
      cmd="export LD_LIBRARY_PATH=/usr/lib64; $ib_write_bw -F -e -s $flow_size -n $flow_iter -l 1 -t 1 -p $port $ip_receiver -L $log |tee $output"
      echo "On $sender: execute $cmd"
      ssh -o "StrictHostKeyChecking no" -p 22 $sender $cmd &
    fi
    let cnt="$cnt + 1"
  ...
  for ((i=1; i<=$num_flows; i++)); do
    ...
    if [[ $cnt -eq $num_flows ]]; then
      ...
      cmd="export LD_LIBRARY_PATH=/usr/lib64; $ib_write_bw -F -e -s $flow_size -n $flow_iter -l 1 -t 1 -p $port &> /dev/null"
      echo "On $ip_receiver: execute $cmd"
      ssh -o "StrictHostKeyChecking no" -p 22 $receiver $cmd &
    fi
    let cnt="$cnt + 1"
