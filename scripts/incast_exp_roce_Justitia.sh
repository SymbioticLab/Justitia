...
if [ $1 == "receiver" ]; then
  for node in $(cat nodes); do
    ...
    cmd="$J_daemon 192.168.1.10 1 $gidx"
...
if [ $1 == "sender" ]; then
  for node in $(cat nodes); do
    ...
    if [[ $cnt -eq $num_senders ]]; then
      ...
      cmd="export LD_LIBRARY_PATH=/usr/lib64; $ib_write_lat -F -d $ib_dev -x $gidx -s $lat_size -n $lat_iter -l 1 -t 1 -p $port $ip_dst --log_off 2>&1 |tee $output"
    else
      sleep 1
      output="$out_dir/bw_result_$node.txt"
      log="$out_dir/bw_log_$node.txt"
      cmd="export LD_LIBRARY_PATH=/usr/lib64; $ib_write_bw -F -e -d $ib_dev -x $gidx -s $bw_size -n $bw_iters -l 1 -t 1 -p $port $ip_dst -L $log 2>&1 |tee $output"
    fi
  ...
  for node in $(cat nodes); do
    let port="$port_base + $cnt"
    if [[ $cnt -eq $num_senders ]]; then
      cmd="export LD_LIBRARY_PATH=/usr/lib64; $ib_write_lat -F -d $ib_dev -x $gidx -s $lat_size -n $lat_iter -l 1 -t 1 -p $port &> /dev/null"
    else
      cmd="export LD_LIBRARY_PATH=/usr/lib64; $ib_write_bw -F -e -d $ib_dev -x $gidx -s $bw_size -n $bw_iters -l 1 -t 1 -p $port &> /dev/null"
    fi
