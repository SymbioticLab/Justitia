...
if [ $1 == "sender" ]; then
  for node in $(cat nodes); do
    ...
    if [[ $cnt -eq $num_senders ]]; then
      ...
      cmd="$ib_write_bw -F -e -d $ib_dev -x $gidx -s $((bw_size*1000)) -n $((bw_iters/1000)) -l 1 -t 1 -p $port -S 3 $ip_dst -L $log |tee $output"
    elif [[ $cnt -eq 3 ]] || [[ $cnt -eq 4 ]]; then
      sleep 1
      output="$out_dir/bw_result_dcqcn_$node.txt"
      log="$out_dir/bw_log_dcqcn_$node.txt"
      cmd="$ib_write_bw -F -e -d $ib_dev -x $gidx -s $((bw_size*100)) -n $((bw_iters/100)) -l 1 -t 1 -p $port -S 3 $ip_dst -L $log |tee $output"
    elif [[ $cnt -eq 5 ]] || [[ $cnt -eq 6 ]]; then
      sleep 1
      output="$out_dir/bw_result_dcqcn_$node.txt"
      log="$out_dir/bw_log_dcqcn_$node.txt"
      cmd="$ib_write_bw -F -e -d $ib_dev -x $gidx -s $((bw_size*10)) -n $((bw_iters/10)) -l 1 -t 1 -p $port -S 3 $ip_dst -L $log |tee $output"
    else
      sleep 1
      output="$out_dir/bw_result_dcqcn_$node.txt"
      log="$out_dir/bw_log_dcqcn_$node.txt"
  ...
  for node in $(cat nodes); do
    ...
    if [[ $cnt -eq $num_senders ]]; then
      ...
      cmd="$ib_write_bw -F -e -d $ib_dev -x $gidx -s $((bw_size*1000)) -n $((bw_iters/1000)) -l 1 -t 1 -p $port -S 3 &> /dev/null"
    elif [[ $cnt -eq 3 ]] || [[ $cnt -eq 4 ]]; then
      cmd="$ib_write_bw -F -e -d $ib_dev -x $gidx -s $((bw_size*100)) -n $((bw_iters/100)) -l 1 -t 1 -p $port -S 3 &> /dev/null"
    elif [[ $cnt -eq 5 ]] || [[ $cnt -eq 6 ]]; then
      cmd="$ib_write_bw -F -e -d $ib_dev -x $gidx -s $((bw_size*10)) -n $((bw_iters/10)) -l 1 -t 1 -p $port -S 3 &> /dev/null"
    else
