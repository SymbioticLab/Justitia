...
//  struct ibv_mr *send_mr;  // not used on client side in single direction communication
...
//  struct ibv_mr *rdma_remote_mr;  // again not used
  ...
  struct message *send_msg;  // only used here to send "MSG_DONE"
...
//    IBV_ACCESS_LOCAL_WRITE));  // since we are send a zero-byte meg, no need to reg mr ////NOTE: test this
